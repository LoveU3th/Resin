...
* 熔断恢复：熔断后的节点依然保留在全局池中，接受 ProbeManager 的主动探测。但**必须等冷却期结束**才能恢复：
	...
	* `scanLatency` 中半开节点**绕过** `MaxLatencyTestInterval` 闸门直接入队。否则冷却期内不会有任何探测，配置成 30s 的冷却实际上要等最多 1 小时才可能被验证，冷却形同虚设。
		* 但绕过**不是无条件**：同一节点的两次半开探测至少间隔 `冷却 / 4`（下限 30 秒）。因为 `CircuitOpenSince` 保持首次隔离时刻不变，冷却一旦涨到 `circuit_max_cooldown` 就**永远**满足"冷却已过"，节点会永久停在半开态；没有这个下限，死节点会被每个扫描周期（约 15 秒）重试一次，永不停止。
		* 间隔跟随节点当前冷却，因此是自适应：刚隔离的节点（冷却 2 分钟）每 30 秒验证一次，恢复很快；长期失败、冷却封顶的节点自动降到每 7.5 分钟一次，不再空耗。
...
### 请求级 failover
一个节点在建立连接阶段失败时，请求会自动换到另一个节点，而不是把 502 直接抛给客户端。

* **重试边界（关键约束）**：只在**连接建立阶段**重试，即请求尚未到达上游时。判定有且仅有两条规则：
...
### 粘性租约与排除
请求重试时必须避开已经失败过的节点，这与粘性租约存在张力，处理原则如下。

* **借出（borrow）而非搬迁**：若粘性节点被本次请求排除，则为本次请求单独借出另一个节点，返回 `CancelOp` **不写租约**、不发 lease 事件。优先选择同一出口 IP 下的其他节点，尽量保持账号出口地址不变。
* **候选用尽时保留租约**：若排除后已无任何节点可用（单节点平台即会触发），必须**保留原租约**。删除租约会让账号在后续请求中被重新分配，等于一次失败就打散了出口 IP——这正是 borrow 要避免的。
* **租约命中也看健康分**：粘性节点若**已确认**不健康（样本数达到 `health_min_samples_for_filter`，且分数低于 `health_filter_threshold_percent`），本次请求同样走借出——换一个节点服务这一次，租约不动。判定门槛与 P2C 的候选过滤完全一致，因此「健康」在系统里只有一条线，两处不会互相矛盾。
	* 判定必须放在**记录命中之前**：否则会为一个实际没有服务的请求发出 `LeaseTouch`，破坏「borrow 不发 lease 事件」的前提。
	* **样本不足的节点不在此列**：`-`（未测量）不等于坏，它只在打分上承担未知惩罚。未知降权、已知坏才停用，两者各管一段。
	* 借出还有一个**前提**：租约节点本身仍算「命中」——还在平台可路由视图内、出口 IP 未变。否则不走借出，照旧走同 IP 轮换或重建租约。**没有这个前提，一个已经下线或换了 IP 的坏节点会把账号钉住整整一个 TTL**，比不借更糟。
	* **借不到就照常服务**：没有任何节点可借时（单节点平台，或所有备选同样被过滤），回退到正常的租约命中。健康度只能让请求换个节点，不能让请求失败，也不能制造租约抖动。
	* 借出不刷新 `LastAccessedNs`，租约会自然到期；到期后重新选路时健康分参与过滤与打分，坏节点由此被自己换掉，无需等到被熔断摘除。
* 一次失败的代价通常只是「这次请求换了个节点」。**唯一的例外是**连接阶段**的失败**：那说明租约节点根本不可达，账号会被搬到合格节点上（见下节）。继续把一个账号钉在连不上的节点上，只会让它的每次请求都先失败一次。

### 连接失败后的租约迁移
borrow 解决了「本次请求怎么绕开坏节点」，却没有解决「这个账号为什么每次都撞上它」。租约一旦落在中低分节点上，就只在分数掉到 `health_filter_threshold_percent` 以下时才会被借出——在那之前，账号每次请求都先失败一次，再靠 failover 救回来。因此**连接阶段的失败必须能让租约搬走**。

* **触发条件只有一个：连接阶段失败**（`connect`）。即「节点不可达 / 连上了但一个字节都没写出去」，与请求级 failover 的 R1、R2 是同一条线。
	* 传输阶段的失败（请求已发出）与「慢」失败**都不触发**：节点已经被摸到了，慢的可能是源站，把账号搬走等于为一个未必属于节点的责任改掉出口地址。
	* 记录**失败的那个节点必须就是租约节点**。在借出节点上失败与租约无关，不触发迁移——这也让「本次请求的重试」天然不会误伤租约。
* **迁移发生在下一个新请求**，不是本次请求的重试：本次请求该借就借（见上节），租约在下一个新请求到来时一次性搬完。这样出口 IP 只变一次，而不是每次重试都变。
* **搬走 = 写入新租约**（`LeaseReplace` 事件），不是借出。迁完继续保持粘性，即「换一次，然后稳下来」。
* **严格池必须真的从合格集合里抽**。与 P2C 的候选过滤不同，这里**不能**在抽不到时退回原节点：退回等于什么都没做。
* **分层兜底**：`sticky_strict_threshold_percent`（默认 85）→ `sticky_strict_fallback_percent`（默认 70）→ 全体（仍排除刚失败的节点）。第三层是必要的：重启后所有节点样本归零，若不设第三层，一个刚重启的实例将无法把任何租约从坏节点上搬走。
* **未测量的节点不参与前两层**，这与过滤逻辑「样本不足一律放行」正好相反，也是这套策略要单独实现的原因：**过滤不敢判定未知节点，而迁移不能把「没有记录」当成「健康」**。阈值与样本线同样取 `health_min_samples_for_filter`。
* **优先同出口 IP 的合格节点**：换节点对账号是真实影响，能留在原地址上就不动出口 IP。
* **冷却**：失败即标记，标记带截止时间（`sticky_strict_cooldown`，默认 5 分钟）。迁成功后标记清除；**迁不动时标记保留但不生效**，因此节点池里没有合格节点时，不会每个请求都重扫一遍可路由视图。截止时间过后再失败会重新标记。
* **绝不能因为迁移失败而让请求失败，也不能制造租约抖动**：严格池与全体都为空（单节点平台即会触发）时保留原租约照常服务。
* 迁移**不修改 `Expiry`**（`CreatedAt` 也不动）：换节点不该顺带延长一个账号的粘性时长。出口 IP 若变化，`IPLoadStats` 随之增减；同地址换节点则计数不变。
* 迁移只由**正向代理**的失败信号触发；反向代理不参与（`RESIN_FORWARD_STICKY_ACCOUNT` 只管正向代理，反向代理的 Account 提取与租约行为不变）。

### 稳定性可观测
failover 会带来一个副作用：请求最终成功了，于是「成功率」看起来一切正常，
而第一个节点其实已经坏了。因此可观测性必须区分这两个口径。

* **首跳成功率**（`first_hop_success / total_requests`）：首次尝试即由第一个
  节点完成的请求占比。**与最终成功率的差值就是 failover 掩盖的故障量**，
  也是判断节点是否「能通但总断」的主要依据。
    * 分子只在「首次尝试的节点即完成请求」时 +1；任何重试过、或最终失败的
      请求都不计入。
    * 分子只在「首次尝试的节点即完成请求」时 +1；任何重试过、或最终失败的
      请求都不计入。
    * **分母是「经过节点的请求数」（`node_requests`），不是总请求数**：
      bypass 流量不经过任何节点，若计为完美首跳会把该比率往 100% 拉，反而
      掩盖故障。bypass 请求仍计入总请求数（它是真实流量），但排除在首跳
      统计之外。
    * 全局与 per-platform 双口径，随 bucket 一同落库（`node_requests` 与
      `first_hop_success` 两列，对已有库幂等添加）。
    * 读路径已打通：`QueryRequests` 返回两列，`QueryHistoryRequests` 合并
      当前未落盘的桶，API 输出 `node_requests` / `first_hop_success` /
      `first_hop_success_rate`。
* **请求日志的 failover 追踪**：`failover_attempts` 记录该请求试过几个节点，
  1 表示首个节点即完成；`failover_nodes` 记录此前失败节点的 hash（最多 3 个、
  每个截断至 16 字符），用于定位具体是哪个节点在拖后腿。
* **节点成功率曝光**：节点列表返回 `success_rate`（即阶段二的健康度 EWMA）
  与 `health_samples`。
    * 样本数不足（`health_min_samples_for_filter`）时**不返回** `success_rate`
...
* **metrics.db 保留策略**：新增 `RESIN_METRIC_RETENTION_DAYS`（默认 30）。
  ...
  bucket 边界整桶删除；设为 0 表示永久保留。清理必须覆盖**所有** bucket 表
  （含节点池与租约寿命），漏掉任何一张它仍会无界增长。
...
* LastAccessed：最后访问时间
* NeedsRebind / StrictUntil：连接失败的标记与其截止时间。**仅内存，不落库**——它由健康状态派生，重启后与健康分一同归零。
...
* 查到表项，且带有**连接失败标记**（未过期）：按「连接失败后的租约迁移」重选合格节点并就地改写租约；找不到合格节点则保留标记、照常服务。
...
在上面算出的评分之上再加一项惩罚，三种分配策略一视同仁：

$\text{Penalty} = \max\big((1 - \text{HealthScore}) \times \text{HealthPenaltyMs},\ \text{未知惩罚}\big)$

* **已知惩罚**：健康度越低罚得越多，满分节点一分不罚。因此**全健康的机队评分与健康度功能上线前完全一致**。
* **未知惩罚**：样本数不足 `health_min_samples_for_filter`（默认 8）的节点，健康度初值是 1.0，只看分数的话它会与跑了上千次的老节点平起平坐。因此按"还差几个样本"线性收费：

	$\text{未知惩罚} = \Big(1 - \frac{\text{HealthSamples}}{\text{health\_min\_samples\_for\_filter}}\Big) \times \text{HealthPenaltyMs}$

	零样本收全额，攒够样本免收。
* 两者取大值而非求和：已经证明不行的节点，不为同一份疑虑罚两次。
* `health_penalty_ms = 0` 时两项都不生效——该开关关闭整个健康惩罚子系统；`health_min_samples_for_filter = 0` 时无法定义"未测量"，只按已知惩罚计。

> **过滤与打分的分工**：过滤只拒绝"已确认差"的节点（样本够且分数低于 `health_filter_threshold_percent`），未测量的节点一律放行；打分负责让未测量的节点排在已验证健康的节点之后。两者判定"未测量"用的是同一条线（`health_min_samples_for_filter`），因此不会互相矛盾。
>
> 重启后所有节点的样本归零，于是一起交同样的押金，相对排序不变——这是该惩罚不会在重启后改变行为的原因。新节点也不会被永久冷落：主动探测遍历整个节点池且结果计入健康度，与是否被路由选中无关，攒够样本即自动退坡。

评分越低越优。

> **健康过滤是「尽力而为」，严格池不是**：过滤在抽不到合格节点时会退回原选择，因为路由失败比选到差节点更糟；而「连接失败后的租约迁移」要求候选真的来自合格集合，因此在严格池里用的是同一套评分、不同的候选构造：先在合格集合内随机取两个再比评分，集合为空则逐层放宽，**绝不退回刚失败的节点**。两者的差异是刻意的，不是实现不一致。
...
| 连接阶段失败（下一个新请求） | 在严格池内重选节点，就地改写 Lease（`Expiry` 不变），发 `LeaseReplace` |
...
#### 粘性租约的严格池
见「连接失败后的租约迁移」。
* `StickyStrictRebindEnabled`：连接阶段失败后是否把租约搬到合格节点。默认 True。
* `StickyStrictThresholdPercent`：合格节点的成功率下限。默认 85。**与 `HealthFilterThresholdPercent` 不同，这里是硬条件**，且样本不足的节点不算合格。
* `StickyStrictFallbackPercent`：第一层为空时的第二层阈值。默认 70，必须不高于阈值。第二层也为空时回落到全体可路由节点（仍排除刚失败的节点）。
* `StickyStrictCooldown`：失败标记的有效时长。默认 5 分钟，**必须为正**——截止时间是「迁不动时不反复扫描」的保证，没有它这个开关就等于每请求重扫一遍。要关闭该功能请用 `StickyStrictRebindEnabled`。
* 合格判定所需的样本数与 `HealthMinSamplesForFilter` 共用，不另设开关。
...
* 版本清单：**v1** 节点健康度与请求级 failover；**v2** 粘性租约严格池（`StickyStrict*`）。v2 以 `StickyStrictThresholdPercent` 作为「是否配置过」的哨兵，理由与 failover 用 `FailoverMaxAttempts` 相同：两个布尔开关无法区分「字段缺失」与「显式关闭」。
